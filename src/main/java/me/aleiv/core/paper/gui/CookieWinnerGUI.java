...
public class CookieWinnerGUI {
    ...
    public CookieWinnerGUI(Player player) {
        this.plugin = Core.getInstance();
        this.player = player;

        this.gui = new ChestGui(6, ".");
        this.titleTemplate = "&8(%p%/%m%) &a&lCookie Winner Menu";
    ...
    private PaginatedPane getPlayerPages() {
        ...
        pane.populateWithGuiItems(
                this.plugin.getGame().getCookieGame().getAllCapules().stream().filter(CookieCapsule::isDone).map(cc -> new GuiItem(this.getPlayerItem(cc), e -> {
                    e.setCancelled(true);
                    Bukkit.getScheduler().scheduleSyncDelayedTask(this.plugin, () -> this.player.teleport(cc.getPlayer().getLocation()), 3L);
    ...
    private ItemStack getPlayerItem(CookieCapsule cookieCapsule) {
        ...
        List<String> lore = List.of("&aPlayer won", "&cFails: &f&l" + cookieCapsule.getErrors(), "&f ", "&7Click to teleport to this player");
