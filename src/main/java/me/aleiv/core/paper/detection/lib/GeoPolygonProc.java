...
public class GeoPolygonProc {
    ...
    public ArrayList<GeoFace> getFaces() {
        return this.Faces;
    }

    public ArrayList<GeoPlane> GetFacePlanes() {
    ...
    private void Set3DPolygonUnitError(GeoPolygon polygon) {
        this.MaxDisError = ((Math.abs(this.X0) + Math.abs(this.X1) + Math.abs(this.Y0) + Math.abs(this.Y1)
    ...
    private void SetConvex3DFaces(GeoPolygon polygon) {
            ...
            for (int j = i + 1; j < n; j++) {
                ...
                for (int k = j + 1; k < n; k++) {
                    ...
                    if (onLeftCount == 0 || onRightCount == 0) {
                        ...
                        if (!CheckIfContainsList(faceVerticeIndex, verticeIndexInOneFace)) {
        ...
        for (int i = 0; i < numberOfFaces; i++) {
            // return face planes
            facePlanes.add(new GeoPlane(fpOutward.get(i).getA(), fpOutward.get(i).getB(), fpOutward.get(i).getC(),
                    fpOutward.get(i).getD()));
            ...
            for (int j = 0; j < count; j++) {
                ...
                gp.add(new GeoPoint(vertices.get(vi.get(j)).getX(), vertices.get(vi.get(j)).getY(),
    ...
    private static boolean CheckIfContainsList(ArrayList<ArrayList<Integer>> faceVerticeIndex,
            ArrayList<Integer> verticeIndexInOneFace) {
        return faceVerticeIndex.stream().anyMatch(p -> p.equals(verticeIndexInOneFace));
    }
}